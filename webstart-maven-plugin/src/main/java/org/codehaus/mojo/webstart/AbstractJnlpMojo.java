...
import org.codehaus.mojo.webstart.generator.*;
...
public abstract class AbstractJnlpMojo
	...
	/**
	 * Default name of the jnlp file.
	 */
	private static final String LAUNCH_JNLP = "launch.jnlp";
	...
	protected boolean attachArchive;
	 ...
	 */
	@Parameter
	protected List<JnlpExtension> jnlpExtensions;
	...
	protected boolean outputJarVersions;
	 ...
	 */
	protected List<Artifact> packagedJnlpArtifacts = new ArrayList<Artifact>();

	/**
	 * the artifacts associated to each jnlp extension
	 */
	protected Map<JnlpExtension, List<Artifact>> extensionsJnlpArtifacts = new HashMap<JnlpExtension, List<Artifact>>();

	protected Artifact artifactWithMainClass;
	...
	public void execute()
		...
		try {
			...
			generateJnlpFile(getWorkDirectory(), jnlp.getOutputFile());
	...
	protected void processDependencies()
		...
		Collection<Artifact> artifacts = isExcludeTransitive() ? getProject().getDependencyArtifacts() : getProject().getArtifacts();
	...
	protected void processDependency(Artifact artifact)
		throws MojoExecutionException {
		// TODO: scope handler
		...
		if (!Artifact.SCOPE_SYSTEM.equals(artifact.getScope()) &&
			...
			if ("jar".equals(type) || "ejb-client".equals(type)) {
				...
				if (toCopy == null) {
					getLog().warn("artifact with no file: " + artifact);
					getLog().warn("artifact download url: " + artifact.getDownloadUrl());
					getLog().warn("artifact repository: " + artifact.getRepository());
					getLog().warn("artifact repository: " + artifact.getVersion());
	...
	protected void generateJnlpFile(File outputDirectory, String outputFile)
		throws MojoExecutionException {
		// ---
		// get output file
		// ---
		...
		if (StringUtils.isBlank(outputFile)) {
			...
			outputFile = LAUNCH_JNLP;
		}
		File jnlpOutputFile = new File(outputDirectory, outputFile);
	...
	protected void processNativeLibs() {
	      /*
	 ...
	 * @throws MojoExecutionException
	 */
	protected void processExtensionsDependencies()
		throws MojoExecutionException {
